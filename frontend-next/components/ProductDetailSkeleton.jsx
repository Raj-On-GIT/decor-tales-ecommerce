    ...
    <div className="mx-auto max-w-screen-xl px-4 pt-5 sm:px-6">
      <div className="mb-10 grid grid-cols-1 items-start gap-8 sm:gap-10 md:grid-cols-2 md:gap-12 md:px-6 lg:px-12">
        <div className="flex flex-col items-center">
          <div className="aspect-square w-full overflow-hidden rounded-xl bg-gray-200 animate-pulse" />

          <div className="mt-4 flex w-full gap-2 sm:mt-5 sm:gap-3">
            {Array.from({ length: 5 }).map((_, index) => (
              <div
                key={index}
                className="h-14 w-14 flex-shrink-0 rounded-lg bg-gray-200 animate-pulse sm:h-15 sm:w-15"
              />
            ))}
          </div>
        </div>
      ...
      <section className="mb-10 mt-16 sm:mt-24">
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-8 md:gap-10 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, index) => (
            <div
              key={index}
              className="rounded-2xl border border-gray-200 bg-[#F0FFDF] p-2 shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
            >
